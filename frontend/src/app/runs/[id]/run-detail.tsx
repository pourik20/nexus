...
export function RunDetail({ id, initialData }: { id: string; initialData: JobRun }) {
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-semibold font-mono text-base">Run {data.id.slice(0, 8)}</h1>
            <RunStatusBadge status={data.status} />
          </div>
          <p className="text-sm text-zinc-500 mt-1">
            Pipeline:{" "}
            <Link href={`/pipelines/${data.pipelineId}`} className="hover:underline">{data.pipelineId}</Link>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 gap-4 text-sm">
          ...
          <Field label="Duration" value={formatDuration(data.startedAt, data.finishedAt)} />
          <Field label="Records processed" value={data.recordsProcessed} />
          {data.errorMessage && (
            <div className="col-span-2">
              <div className="text-xs uppercase tracking-wide text-zinc-500">Error</div>
              <pre className="mt-1 text-xs text-red-700 dark:text-red-300 whitespace-pre-wrap">{data.errorMessage}</pre>
            </div>
          )}
        </CardContent>
      </Card>
            ...
            <TBody>
              {steps.map((s) => (
                <TR key={s.id}>
                  ...
                  <TD><RunStatusBadge status={s.status} /></TD>
      ...
      <Card>
        <CardHeader><CardTitle>Version snapshot</CardTitle></CardHeader>
        <CardContent>
          <pre className="text-xs font-mono whitespace-pre-wrap break-words">{JSON.stringify(data.pipelineVersionConfigSnapshot, null, 2)}</pre>
        </CardContent>
      </Card>
    </div>
  );
