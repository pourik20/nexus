...
export const runsApi = {
  list: async (params?: ListRunsParams, init?: RequestInit): Promise<JobRun[]> =>
    handle(await fetch(`${API_BASE_URL}/runs${qs(params)}`, { cache: "no-store", ...init })),

  get: async (id: string, init?: RequestInit): Promise<JobRun> =>
    handle(await fetch(`${API_BASE_URL}/runs/${id}`, { cache: "no-store", ...init })),

  triggerForPipeline: async (pipelineId: string): Promise<JobRun> =>
    handle(await fetch(`${API_BASE_URL}/pipelines/${pipelineId}/run`, { method: "POST" })),

  patch: async (id: string, body: PatchRunRequest): Promise<JobRun> =>
    handle(await fetch(`${API_BASE_URL}/runs/${id}`, {
      method: "PATCH",
